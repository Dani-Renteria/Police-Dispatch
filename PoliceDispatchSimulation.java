...
public class PoliceDispatchSimulation {
    ...
    public void addToIncidentQueue(Incident incident) {
        this.eventQueue.add(new HeapElement(incident), incident.getTimePriority());
    }
    ...
    public void run() {
        ...
        while (eventQueue.size() != 0) {
            while (eventQueue.peek().value.getReportTime() == time) {
                if (eventQueue.peek() == null){
                    break;
                }
                if (!eventQueue.peek().value.dispatched()) {
                    HeapElement element = new HeapElement(eventQueue.poll().value);
                    logReport(element.value.getReportTime(), element.value);
                    triageQueue.add(element, element.value.getTriagePriority());

                }
                else {
                    while (eventQueue.peek() != null && eventQueue.peek().value.dispatched()) {
                        if (!eventQueue.peek().value.dispatched()){
                            break;
                        }
                        HeapElement element = new HeapElement(eventQueue.poll().value);
                        logResolution(element.value.getResolutionTime(), element.value);
                        markUnitAvailable(element.value.getDispatchedUnit());
                        time = element.value.getResolutionTime();
                    }
                    break;
                    }
                }
            while (nextAvailableUnit() != -1 && triageQueue.size() != 0) {
                if (nextAvailableUnit() == -1){
                    break;
                }
                if (triageQueue.size() == 0){
                    break;
                }
                HeapElement triElement = new HeapElement(triageQueue.poll().value);
                int unit = nextAvailableUnit();
                triElement.value.dispatch(time, unit);
                logDispatch(time, triElement.value);
                markUnitUnavailable(unit);
                eventQueue.add(triElement, triElement.value.getResolutionTime());
                }
            time++;
            }
        }
